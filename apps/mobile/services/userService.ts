...
export const updateUserOnlineStatus = async (userId: string, online: boolean) => {
  const { data, error } = await supabase
    .from('users')
    .update({ online })
    .eq('id', userId);
...
export const subscribeToUserStatus = (userId: string, callback: (online: boolean, lastSeen: string | null) => void) => {
  const subscription = supabase
    .channel(`public:users:id=eq.${userId}`)
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'users', filter: `id=eq.${userId}` }, (payload) => {
      const { online, last_seen } = payload.new;
